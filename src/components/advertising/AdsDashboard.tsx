...
export function AdsDashboard({ keywords, bookEconomy }: AdsDashboardProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <MetricCard
          title="Gasto Total"
          value={formatearMoneda(metrics.totalGasto)}
          subtitle={`${metrics.keywordsWithAdsData} keywords con datos`}
          icon={<DollarSign className="w-5 h-5 text-muted-foreground" />}
          tooltip="Suma de todos los gastos de Ads en las keywords"
        />
        
        <MetricCard
          title="Ventas Totales"
          value={formatearMoneda(metrics.totalVentas)}
          subtitle={`${metrics.totalPedidos} pedidos`}
          icon={<ShoppingBag className="w-5 h-5 text-muted-foreground" />}
          tooltip="Ingresos totales por ventas atribuidas a Ads"
        />
        
        <MetricCard
          title="Beneficio"
          value={formatearMoneda(metrics.totalBeneficio)}
          subtitle={metrics.totalBeneficio >= 0 ? "Rentable" : "En pérdidas"}
          icon={metrics.totalBeneficio >= 0 
          ...
          tooltip="Ventas totales menos gasto total"
        />
        
        <MetricCard
          title="ACOS Total"
          value={formatearPorcentaje(metrics.acosTotal)}
          subtitle={acosVsEquilibrio === 'profitable' 
            ? "Por debajo del PE ✓" 
            : acosVsEquilibrio === 'losing' 
            ? "Por encima del PE ⚠" 
            : "—"
          }
          icon={<Target className="w-5 h-5 text-muted-foreground" />}
          valueClassName={cn(
            acosVsEquilibrio === 'profitable' 
              ? "text-green-600 dark:text-green-400" 
              : acosVsEquilibrio === 'losing' 
              ? "text-amber-600 dark:text-amber-400" 
              : ""
          )}
          tooltip="Advertising Cost of Sales global. Compara con el ACOS de punto de equilibrio."
        />
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <MetricCard
          ...
          tooltip="Suma de todos los clicks de Ads"
        />
        ...
        <MetricCard
          ...
          tooltip="Porcentaje de clicks que resultan en pedido"
        />
        
        <MetricCard
          title="KW Rentables"
          value={metrics.profitableKeywords.toString()}
          subtitle={`${((metrics.profitableKeywords / Math.max(metrics.keywordsWithAdsData, 1)) * 100).toFixed(0)}% del total`}
          icon={<CheckCircle2 className="w-5 h-5 text-green-500" />}
          valueClassName="text-green-600 dark:text-green-400"
          tooltip="Keywords con beneficio positivo"
        />
        
        <MetricCard
          title="KW En pérdidas"
          value={metrics.losingKeywords.toString()}
          subtitle={`${((metrics.losingKeywords / Math.max(metrics.keywordsWithAdsData, 1)) * 100).toFixed(0)}% del total`}
          icon={<XCircle className="w-5 h-5 text-red-500" />}
          valueClassName="text-red-600 dark:text-red-400"
          tooltip="Keywords con beneficio negativo"
        />
      </div>

      {/* Rentability Distribution */}
      {metrics.keywordsWithAdsData > 0 && (
        <Card>
